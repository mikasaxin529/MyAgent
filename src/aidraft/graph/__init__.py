...
本模块用 langgraph 的 StateGraph 做编排，核心能力：
...
3. 流式产出：节点内调 gateway.stream_chat，逐 token yield ChatChunk，
   经 web 层转成 SSE 帧推给前端——ChatGPT 式流式 + 思考过程。
4. 可观测：每个节点进出经 emitter 推 {type:"node", node_id, status}，
   驱动前端时间线渲染。
5. 记忆：extract_memory / compress_memory 节点把长期事实与历史摘要
   落 SQLite（.aidraft/store.db），跨会话可回读。

设计选型（为什么用 langgraph 而非手写编排）：
StateGraph 的"状态 + 条件边"是声明式编排，新增 agent 分支只加节点+条件
映射，不改既有节点。general 智能体（agenthub/general）的图即委托本模块
的 build_chat_graph 组装。
"""
