"""confirm 节点：大纲确认 / 主题切换 / 自然语言改纲（跨轮状态机第二环）。

图每轮新建实例，state 从零开始——大纲靠查盘（state.json）恢复。
意图判定用确定性关键词（确认词表 / 主题词表），只有"改纲"才调 LLM：
...
def _make_confirm_node(gateway: Any, emitter: Callable[[dict], None] | None,
    ...
    async def confirm(state: YuwenState) -> dict:
        ...
        if is_confirm:
            ...
            detail = f"大纲已确认（主题 {theme}）" if theme else "大纲已确认"
        ...
        _save_state(params, yuwen_outline=edited)
